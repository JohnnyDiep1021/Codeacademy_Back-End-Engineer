...
const getWorkByMinionId = (modelType, instance) => {
  const model = getAllFromDatabase(modelType);
  return model.filter((element) => element.minionId === instance.id);
};

// const updateWorkById = (instance) => {
//   const model = findDataArrayByName("work");
//   const instanceIndex = model.data.findIndex(
//     (work) =>
//       Number(work.minionId) === Number(instance.minionId) &&
//       Number(work.id) === Number(instance.id)
//   );
...
const updateInstanceInDatabase = (modelType, instance) => {
  ...
  let instanceIndex;
  if (modelType === "work") {
    instanceIndex = model.data.findIndex(
      (work) =>
        Number(work.minionId) === Number(instance.minionId) &&
        Number(work.id) === Number(instance.id)
    );
  } else {
    instanceIndex = model.data.findIndex((element) => {
